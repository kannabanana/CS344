...
int main(char argc,char * argv[])
{
	...
	portno = atoi(argv[1]);			//opt_enc_d portnumber (port number is 1st argument)
	...
	while(1)
	{
		...
		//make a connnection and accept, so fork after
		...
		if (pid == 0)
		{
			...
			//reading the message
			bzero(buffer_1,buffer);		//sets the first n bytes of the buffer_1 to zero
