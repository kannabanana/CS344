...
int decision(char ** read_input,int usr_continue,int bg);	//this command executes the command so the user's command gets executed (the basic commands and more complex ones) Input: the array, if the user wants to continue, if its a background process
...
do{
	...
	//do the read in here
		...
		usr_continue = decision(read_input1,usr_continue,bg);		//this function returns whether to continue and executes the command the user origionally asks for (main point of the smallsh)
}
