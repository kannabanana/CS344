...
int decision(char ** read_input,int exit_val,int bg)
{
	...
	act.sa_handler = SIG_IGN;	//declaring an instance of sigaction
	...
	else
	{
		...
		else if(processid == 0)		//child process
		{
			...
			//if there is no background process call the standard input files
			if (bg == -1){
			...
			if (ok == 1)		//check if the files returned ok proper 
				return 1;
			else if(bg >= 0)
			{
				int in;
				...
				if (in >=0)
					printf("bg process has a file input\n");	
		...
		else		//parent process
		{
			...
			*/
			...
			{
				printf("not background process-parent\n");
...
//	}
	...
	//check the background process is over before prompting
	pid = waitpid(-1,&status,WNOHANG);
	while(pid > 0){

		printf("backgound process complete %d\n",pid);
		if(WIFEXITED(status))
			printf("Exit status: %d\n",WEXITSTATUS(status));
		else
			printf("Terminating signal %d\n",status);
